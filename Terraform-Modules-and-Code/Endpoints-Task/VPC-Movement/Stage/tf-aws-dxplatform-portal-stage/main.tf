...
cloud {
    ...
    workspaces {
      name = "tf-aws-dxplatform-portal-stage"
    }
  }
...
locals {
  default_tags = {
    Project          = "dxplatformportal-stage"
    Managed          = "Terraform"
    Owner            = "dxplatform"
...
module "ALB" {
  ...
  source = "github.com/sgs-dxc/terraform-aws-alb.git?ref=v1.5"
  ...
  client_id              = "4aoncumcjbcm7ne7toj0akdvtm"
  client_secret          = "1l1vhkc7m88h0e4spf1rifeorh5hgbebopsp23agfnm8kbd3vnns"
  ...
  s3_bucket_prefix = "dxplatformportal-stage"
  # Default Tags
  default_tags  = local.default_tags
  sns_topic_arn = "arn:aws:sns:ap-southeast-1:768502287836:DXPlatformPortal-Static-Staging-EmailSNSTopic-3VXZYU6YDCD8"

}

####

resource "aws_ssm_parameter" "cw_agent_dxplatformportal-stage" {
  description = "Cloudwatch agent config to configure custom log"
  name        = "/dxplatformportal/stage/dxplatformportal-stage/cloudwatch-agent/cw_agent_config"
  type        = "String"
  value       = file("./user_data/dxplatform-portal-stage/cw_agent_config.json")
  tags        = local.default_tags
}


module "autoscale_app" {
  source         = "github.com/sgs-dxc/terraform-aws-ASG.git?ref=v1.2.5"
  BillingTag     = "dxplatform"
  NameTag        = "dxplatformportal-stage"
  Volume_NameTag = "dxplatformportal-stage"
  OwnerTag       = "dxplatform"
  # replace the values accordingly
  ec2_subnet_a                = "subnet-20bdbe69"
  ec2_subnet_b                = "subnet-5c31233b"
  env_prefix                  = local.envPrefix
  image_id                    = "ami-07a423a1374be86f0"
  instance_type               = "t2.micro"
  key_pair_name               = "DXInfra"
  asg_instance_profile_prefix = "dxplatformportal-stage"
  desired_capacity            = "1"
  max_instance_size           = "1"
  min_instance_size           = "1"
  monitor                     = true
  ...
  userDataDetails = base64encode(templatefile("./user_data/dxplatform-portal-stage/userdata_to_ec2.sh", { ssm_cloudwatch_config = aws_ssm_parameter.cw_agent_dxplatformportal-stage.name }))
