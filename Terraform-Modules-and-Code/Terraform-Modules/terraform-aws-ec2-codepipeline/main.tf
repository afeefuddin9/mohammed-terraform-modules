...
resource "aws_codebuild_project" "codebuild_project" {
  count         = var.create_codebuild_project ? 1 : 0
...
resource "aws_codedeploy_deployment_group" "codedeploy_deployment_group" {
  count                  = var.create_codedeploy_app ? 1 : 0
  app_name               = aws_codedeploy_app.codedeploy_application[count.index].name
...
resource "aws_codepipeline" "codepipeline" {
  ...
  dynamic "stage" {
    for_each = var.Stage_build_isEnabled ? toset([1]) : toset([])
    content {
    name = "Build"
    dynamic "action" {
      for_each = var.Stage_build_isEnabled ? toset([1]) : toset([])
      ...
      configuration = {
        ProjectName = element(concat(aws_codebuild_project.codebuild_project.*.name,  [""]), 0)
      }
     }
    }
   }
 ...
 #define stage for deployment
  dynamic "stage" {
    for_each = var.Stage_Deploy_isEnabled ? toset([1]) : toset([])
    content {
      name = "Deploy"
      dynamic "action" {
        for_each = var.Stage_Deploy_isEnabled ? toset([1]) : toset([])
        content {
          ...
          configuration = {
            ApplicationName     = element(concat(aws_codedeploy_app.codedeploy_application.*.name,  [""]), 0)
            DeploymentGroupName = element(concat(aws_codedeploy_deployment_group.codedeploy_deployment_group.*.deployment_group_name,  [""]), 0)
          }
        }
      }
    }
